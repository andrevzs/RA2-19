...
class Parser:
    ...
    def _casa_com(self, terminal_esperado):
        ...
        Args:
            terminal_esperado: terminal que se espera (tipo ou valor direto)
            
        Returns:
            NodoArvore representando o terminal
            
        Raises:
            ErroSintaticoException: se o token não corresponder
        """
        token_tipo = self._token_atual()
        token_valor = self._valor_token_atual()
        
        # Compara por tipo primeiro
        if token_tipo == terminal_esperado:
            nodo = NodoArvore("terminal", terminal_esperado)
            self._avancar()
            return nodo
        
        # Se não for um tipo, tenta comparar com valor direto
    ...
    def parsear(self):
        ...
        Returns:
            NodoArvore: raiz da árvore sintática
            
        Raises:
            ErroSintaticoException: se houver erro sintático
        """
        try:
    ...
    # ============================================================
    # Funções para cada não-terminal (parsing descendente recursivo)
    # ============================================================
    ...
    def _parsear_listalinhas(self):
        ...
        Usa FIRST/FOLLOW para determinar qual produção aplicar.
        """
        nodo = NodoArvore("nao_terminal", "listalinhas")
        
        token_tipo = self._token_atual()
        
        # Verifica se deve expandir (FIRST(linha) ∩ {token_atual} ≠ ∅)
        # linha começa com LPAREN
        if token_tipo == "LPAREN":
            nodo.add_filho(self._parsear_linha())
            nodo.add_filho(self._parsear_listalinhas())
        else:
            # ε - não faz nada, produto vazio
            pass
    ...
    def _parsear_corpoaposvalor(self):
        ...
        # FIRST(valor operador) = {ID, INT, REAL, LPAREN}
        if token_tipo in ["INT", "REAL", "ID", "LPAREN"]:
            nodo.add_filho(self._parsear_valor())
            nodo.add_filho(self._parsear_operador())
        else:
            # ε - produção vazia quando temos RPAREN ou EOL
    ...
    def _parsear_operador(self):
        ...
        # Se o tipo é OPERADOR ou o valor é operador válido
    ...
    def _parsear_listacomando(self):
        ...
        # FIRST(linha) = FIRST(comando) = {LPAREN}
        if token_tipo == "LPAREN":
            nodo.add_filho(self._parsear_linha())
            nodo.add_filho(self._parsear_listacomando())
        else:
            # ε - produção vazia (RPAREN fecha o BLOCK)
            pass
...
def validarGramatica(gramatica_analise):
    ...
    Args:
        gramatica_analise: Resultado de analisarLL1()
...
def testar_parser():
    ...
    # Caso 1: Expressão aritmética simples
    ...
    # Caso 5: Erro sintático - parêntese não balanceado
